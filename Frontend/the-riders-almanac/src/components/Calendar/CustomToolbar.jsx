...
export default function CustomToolbar({
  ...
  const selectAll = () => { const next = new Set(availableTypes); setChecked(next); emit(next); };
  const clearAll  = () => { const next = new Set(); setChecked(next); emit(next); };

   return (
    <div className="rbc-toolbar ra-toolbar">
  <div className="ra-toolbar__grid">
    ...
    <div className="ra-toolbar__group ra-toolbar__group--left">
      ...
      <Button className="ra-pillbtn ra-pillbtn--today" onClick={() => go("TODAY")}>
        Today
      </Button>
    ...
    <div className="ra-toolbar__group ra-toolbar__group--right">
      ...
      {/* Drawer + scrim (now positioned to .ra-calendar-card) */}
      {open && (
        <>
          <div className="ra-filter-scrim" onClick={() => setOpen(false)} aria-hidden="true" />
          <aside className="ra-filter-drawer" role="dialog" aria-label="Event filters"
                 onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
            <div className="ra-filter-header">
              <strong>Filters</strong>
              <button type="button" className="ra-filter-close" onClick={() => setOpen(false)} aria-label="Close">✕</button>
            </div>
